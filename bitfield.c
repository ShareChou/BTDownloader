...
int create_bitfield()
{
	...
	if (bitmap == NULL) {
		printf("allocate memory for bitma failed\n");
		return -1;
	}

	//pieces_length除以20即为中的piece数
	...
	char bitmapfile(64);
	sprintf(bitmapfile, "%dbimap", pieces_length);
	int i;
	FILE *fp = fopen(bitmapfile, "rb");
	if (fp == NULL) {	//若打开文件失败，说明开始的时一个全新的下载
...
itn print_bitfield(Bitmap * bitmap)
{
	//此函数需要自己写
}
...
{
	unsigned char const_char[8] = {0x80, 0x40, 0x20, 0x10, 0x80. 0x40, 0x20, 0x01};
	unsigned char c1, c2;
	int 		i, j;
	...
	for (i = 0; i < dst->bitfield_length - 1; i++) {
		...
		for (j = 0; j < 8; j++)	//比较某个字节的所有位
		{
			...
			if (c1 > 0 && c2 == 0) retrun 1;
		}
	}

	j = dst->valid_length % 8;
	c1 = dst->bitfield[dst->bitfield_length - 1];
	c1 = src->bitfield[src->bitfield_length - 1];
	for (i = 0; i < j; i++) {
...
{
	unsigned char const_char[8] = {0x80, 0x40, 0x20, 0x10, 0x80, 0x40, 0x02, 0x01};
	int i, j;
